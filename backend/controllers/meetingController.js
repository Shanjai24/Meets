...
const assignResponsibility = async (req, res) => {
    ...
    if (!pointId || !memberId) {
        return res.status(400).json({
            success: false,
            message: `${!pointId ? 'pointId' : 'memberId'} is required`
        });
    }
    ...
    try {
        ...
        // Check if memberId is part of the same meeting
        const [memberResult] = await db.query(
            'SELECT id FROM meeting_members WHERE meeting_id = ? AND id = ?',
            [meetingId, memberId]
        );
        ...
        if (memberResult.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Member is not part of the meeting associated with this point'
            });
        }
...
const getUserMeetings = async (req, res) => {
    ...
    try {
        ...
        // Get meeting details using meeting IDs
        const [meetingDetails] = await db.query(
            `SELECT * FROM meeting WHERE id IN (?);`,
            [meetingIds]
        );

        // Merge role information with meeting details
        const finalMeetings = meetingDetails.map(meeting => ({
            ...meeting,
            role: meetings.find(m => m.meeting_id === meeting.id).role || null
        }));
