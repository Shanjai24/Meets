...
export default function Template() {
    ...
    useEffect(() => {
        axios.get('http://localhost:5000/api/templates/categories')
            .then(response => {
    ...
    useEffect(() => {
        if (location.state?.templates) {
            setTemplates(prevTemplates => [...prevTemplates, ...location.state.templates]);
        }
    ...
    const handleSaveAndNext = () => {
        ...
        navigate('/template', { state: { selectedCategory } }); // Pass selectedCategory to Template.jsx
    };
                                        ...
                                        <tbody>
                                            {filteredTemplates.map((template) => (
                                                <tr key={template.id}>
                                                    ...
                                                    <td>
                                                        <div className={`db-status ${template.status.toLowerCase()}`} onClick={() => handleStatusClick(template.id)}>
                                                            <span className={`db-status-dot ${template.status.toLowerCase()}`} />
                                                            {template.status}
